...
export default {
  ...
  setup(props, {emit}) {
    ...
    const getRecord = async () => {
      loading.value = true;
      await api.get(`/${props.resource}/record/${props.recordId}`)
        .then(response => {
      ...
      title.value = (form.value.is_active?'Suspender: ':'Activar: ')+form.value.text
      loading.value = false;
    };

    const onSubmit = async () => {
      loading.value = true;
      await api.post(`/${props.resource}/state`, form.value)
        .then(response => {
